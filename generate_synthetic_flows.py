...
class SyntheticFlowGenerator:
    ...
    def generate_dataset(self, num_samples=1000, distribution=None):
        ...
        synthetic_flows = []
        print(f"Generating {num_samples} synthetic flows...")
        for label, proportion in distribution.items():
            num_label_samples = int(num_samples * proportion)
            print(f"\nGenerating {num_label_samples} {label} flows...")
            for _ in tqdm(range(num_label_samples)):
                flow_text = self.generate_flow(label=label, temp=0.8)
                if flow_text:
...
def main():
    ...
    synthetic_df = generator.generate_dataset(num_samples=args.num_samples)
